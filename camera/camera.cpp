...
int main(int argc, char *argv[])
{
  ...
  while (1) {
    ...
    printf("Size = %d x %d, Depth = %d, Bytes = %llu\n", frame->size[0], frame->size[1], frame->data_depth, frame->total_bytes);

    Mat img(frame->size[0], frame->size[1], CV_8UC1, frame->image, frame->size[1]);
